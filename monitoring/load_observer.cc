...
// Macro for conditional logging based on options
#define LO_LOG(...)                 \
  do {                              \
...
LoadObserver::LoadObserver(const LoadObserverOptions& options, Env* env)
  ...
  LO_LOG("[LoadObserver] Initialized: window=%lu ms, history_size=%zu\n",
         options_.sampling_window_ms, options_.sample_history_size);
}

LoadObserver::~LoadObserver() { Stop(); }

void LoadObserver::Start() {
  bool expected = false;
  if (running_.compare_exchange_strong(expected, true)) {
    // Start background thread
    bg_thread_ = port::Thread(&LoadObserver::BGThread, this);
    LO_LOG("[LoadObserver] Started monitoring thread\n");
  } else {
    LO_LOG("[LoadObserver] Start called but thread is already running\n");
  }
}
...
void LoadObserver::Stop() {
  ...
  if (running_.compare_exchange_strong(expected, false)) {
    LO_LOG("[LoadObserver] Stopping monitoring thread...\n");
...
void LoadObserver::UpdateStressFactor() {
  ...
  if (stress > 0.7) {
    ...
    LO_LOG("[LoadObserver] HIGH STRESS DETECTED: σ(t)=%.3f\n", stress);
  }

  LO_LOG("[LoadObserver] σ(t)=%.3f (PMem=%.3f, CPU=%.3f)\n", stress,
         pmem_latency_norm, cpu_util);
}
...
double LoadObserver::CalculateNormalizedPMemLatency() {
  ...
  if (pmem_sample_count_ == 0) {
    LO_LOG("[LoadObserver] No PMem samples in window.\n");
    mu->Unlock();
    return 0.0;
  }
  ...
  LO_LOG("[LoadObserver] PMem Stats: avg=%lu ns, count=%lu, baseline=%lu\n",
         avg_latency, pmem_sample_count_, options_.baseline_pmem_latency_ns);
  ...
  double normalized =
      static_cast<double>(avg_latency - options_.baseline_pmem_latency_ns) /
...
double LoadObserver::CalculateCPUUtilization() {
  ...
  LO_LOG("[LoadObserver] CPU Stats: cpu_us=%lu, elapsed_us=%lu, util=%.4f\n",
         total_cpu_time_us_, elapsed_time, utilization);
...
void LoadObserver::RecordPMemLatency(uint64_t latency_ns) {
  ...
  if (pmem_latency_samples_.size() > 10000) {
    LO_LOG("[LoadObserver] Truncating PMem sample buffer\n");
...
void LoadObserver::Calibrate() {
  LO_LOG("[LoadObserver] Calibration started...\n");
  ...
  LO_LOG("[LoadObserver] Calibration complete. Baseline: %lu ns, Max: %lu ns\n",
         options_.baseline_pmem_latency_ns, options_.max_pmem_latency_ns);
}

StressSample LoadObserver::GetCurrentSample() const {
  StressSample sample;
  sample.timestamp_us = env_->NowMicros();
  sample.pmem_latency_normalized =
      pmem_latency_normalized_.load(std::memory_order_acquire);
...
void LoadObserver::GetRecentSamples(std::vector<StressSample>* samples) const {
  ...
  LO_LOG("[LoadObserver] Retrieving %zu samples from history\n", total_samples);

  if (total_samples < options_.sample_history_size) {
    samples->insert(samples->end(), sample_history_.begin(),
