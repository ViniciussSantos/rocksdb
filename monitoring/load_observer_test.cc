...
TEST_F(LoadObserverTest, PMemLatencyRecording) {
  ...
  observer.RecordPMemLatency(500);
...
TEST_F(LoadObserverTest, HighPMemLatency) {
  ...
  double pmem = observer.GetNormalizedPMemLatency();
  double cpu = observer.GetCPUUtilization();

  ASSERT_GT(stress, 0.7);
